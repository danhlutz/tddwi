...
nilNotChar : ValidInput [] -> Void
nilNotChar (Letter _) impossible

manyNotChar : ValidInput (x :: y :: xs) -> Void
manyNotChar (Letter _) impossible

isValidInput : (cs : List Char) -> Dec (ValidInput cs)
isValidInput [] = No nilNotChar
isValidInput (x :: []) = Yes (Letter x)
isValidInput (x :: y :: xs) = No manyNotChar
...
main = do result <- game {guesses=2}
          ...
          case result of
               ...
               (Won (MkWordState word missing)) =>
                    putStrLn ("You won. The word was " ++ word)
